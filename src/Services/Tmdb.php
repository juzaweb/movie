...
class Tmdb
{
    public const API_URL = "http://api.themoviedb.org/3/";
    ...
    {
        $timeZone = $timeZone ?? $this->getConfig()->getTimeZone();
    ...
    {
        $appendToResponse = $appendToResponse ?? $this->getConfig()->getAppender('movie');
    ...
    {
        $appendToResponse = $appendToResponse ?? $this->getConfig()->getAppender('tvshow');
    ...
    {
        $appendToResponse = $appendToResponse ?? $this->getConfig()->getAppender('collection');
    ...
    protected function call($action, $appendToResponse = '')
    {
        ...
        if ($this->getConfig()->getDebug()) {
            echo '<pre><a href="' . $url . '?' . http_build_query($query) . '">check request</a></pre>';
        }

        return Http::throw()->get($url, $query)->json();
    }
}
