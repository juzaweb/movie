    ...
    <form action="{{ $action }}" class="form-ajax" method="post">
        ...
        <div class="row">
            <div class="col-md-12">
                ...
                @if ($model->exists)
                    <a href="{{ admin_url('movies/' . $model->id . '/servers') }}" class="btn btn-info">
            ...
            <div class="col-md-9">
                <x-card title="{{ __('movie::translation.information') }}">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4">
                            {{ Field::number(__('movie::translation.tmdb_rating'), 'tmdb_rating', ['value' => $model->tmdb_rating, 'step' => '0.01', 'min' => '0', 'max' => '10']) }}
                        </div>
                        <div class="col-md-4">
                            {{ Field::number(__('movie::translation.year'), 'year', ['value' => $model->year, 'min' => '1900', 'max' => '2100']) }}
                        </div>
                    </div>
            ...
            <div class="col-md-3">
                ...
                <x-card>
                    {!! Field::select($model, 'status', [
                        'label' => __('movie::translation.status'),
                        'value' => $model->status?->value,
                    ])->dropDownList(\Juzaweb\Modules\Core\Enums\PostStatus::all()) !!}

                    {{ Field::checkbox(__('movie::translation.is_paid'), 'is_paid', ['value' => $model->is_paid ?? false]) }}
                </x-card>
                ...
                <x-card title="{{ __('movie::translation.relationships') }}">
                    {{ Field::tags(__('movie::translation.actors'), 'actors[]', ['value' => $model->actors->pluck('id')->toArray()])->loadDataModel(\Juzaweb\Modules\Movie\Models\Actor::class)->dropDownList($model->actors->pluck('name', 'id')->toArray()) }}

                    {{ Field::tags(__('movie::translation.directors'), 'directors[]', [
                        'value' => $model->directors->pluck('id')->toArray(),
                    ])->loadDataModel(\Juzaweb\Modules\Movie\Models\Director::class)->dropDownList($model->directors->pluck('name', 'id')->toArray()) }}

                    {{ Field::tags(__('movie::translation.writers'), 'writers[]', [
                        'value' => $model->writers->pluck('id')->toArray(),
                    ])->loadDataModel(\Juzaweb\Modules\Movie\Models\Writer::class)->dropDownList($model->writers->pluck('name', 'id')->toArray()) }}
                </x-card>
